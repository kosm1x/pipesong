...
| 2026-06-17 | Deepgram Flux + Pipecat 1.x upgrade    | `BRANCH / UNVALIDATED` | Branch `flux-pipecat1x` (rebuilt from main via `/ship-it-py`): Nova-3→Flux STT, Pipecat 0.0.106→1.4.0, Silero/Smart-Turn→ExternalUserTurnStrategies. Static-only here (no GPU, pipecat not installed, no test suite): `py_compile` pass + carried-forward qa-audit (PASS w/ warnings). NOT run/deployed/merged. Blocking gate C1 = confirm Flux accepts 8 kHz telephony. Gate + diffs: `docs/upgrade-flux-pipecat1x-2026-06-17.md`.                                                                                                                                                                                                                                                                |
| 2026-08-01 | Ecosystem reprobe (no GPU)             | `PLANNED`              | **C1 CLOSED by docs** — Flux quickstart lists 8 kHz + mulaw as supported; W1 (Kokoro `language="es"` string) also closed. **Two latent install breaks found on the branch** (STTMuteFilter removed in pipecat 1.0; flux import needs `.stt` submodule) — the June syntax-only check couldn't see them. New targets: pipecat 1.7.0 (Flux leak+Configure fixes), vLLM 0.25.1 + awq_marlin + native tool calling post-merge (retires ToolCallProcessor), model A/B Qwen3.5-9B-AWQ vs Ministral-3-8B (SpanishOnlyFilter stays — CJK not fixed in Qwen3.x). Flux multi $0.0078/min PAYG ≈ 1.8× Nova-3, target still <$0.03/min. 4-phase plan: `docs/REPROBE-2026-08-01.md`. Phases 0–1 run on this VPS. |
