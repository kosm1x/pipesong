...
class Agent(Base):
    ...
    eot_threshold: Mapped[float | None] = mapped_column(Float, nullable=True)  # Flux EOT confidence 0.5-0.9 (None = Flux default 0.7)
    eager_eot_threshold: Mapped[float | None] = mapped_column(Float, nullable=True)  # 0.3-0.9, must be <= eot_threshold; None = eager OFF (extra LLM-call cost)
    eot_timeout_ms: Mapped[int | None] = mapped_column(Integer, nullable=True)  # Flux EOT silence timeout 500-10000 ms (None = Flux default 5000)
